...
@Composable private fun DragPanel(c:Context,tasks:List<TodoItem>,update:(TodoItem)->Unit){
    ...
    Column(Modifier.fillMaxSize().padding(12.dp)){Text("Drag & Drop واقعی",fontSize=20.sp);LazyColumn{itemsIndexed(order,key={_,t->t.id}){i,t->Card(Modifier.fillMaxWidth().padding(3.dp).pointerInput(order){detectDragGesturesAfterLongPress(onDragStart={drag=i},onDragEnd={drag=-1},onDragCancel={drag=-1}){_,dy->if(kotlin.math.abs(dy.y)>25&&drag>=0){val n=(drag+if(dy.y>0)1 else -1).coerceIn(0,order.lastIndex);if(n!=drag){val m=order.toMutableList();val x=m.removeAt(drag);m.add(n,x);order=m;drag=n}}}}){Row(Modifier.padding(12.dp)){Icon(Icons.Default.DragHandle,null);Text(t.title,Modifier.weight(1f));Text((i+1).toString())}}}};Button({order.forEachIndexed{i,t->update(t);UltimateStore.history(c,t,"order:"+i) }}){Text("ذخیره ترتیب")}}
}
